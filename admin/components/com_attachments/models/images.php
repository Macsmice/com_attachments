...
{
    protected $_base_path; // Site root
    protected $_file_path; // Path to original file
    protected $_original;  // Name of original
    ...
    public function save($config = array())
    {
        ...
        // Retrieve the original filename, strip extension and append new extension
        $parts     = explode('/',$this->_original);
        $filename  = JFile::stripExt($parts[count($parts)-1]);
        $filename .= $this->_getExt($config->mime);
        ...
        switch($config->mime)
        {
            case 'image/png':
                $compression = 0;
                $saved = $method($this->_src, $destination, $compression);
                break;
            case 'image/jpg':
            case 'image/jpeg':
                $quality = 100;
                $saved = $method($this->_src, $destination, $quality);
                break;
            case 'image/gif':
            default:
                $saved = $method($this->_src, $destination);
                break;
    ...
    {
        if(!$file) {
            $file = $this->_base_path.$this->_file_path.$this->_original;
        }
